...
def _quantize(v: int) -> int:
    """Store only values on which validators can reach exact consensus."""
    bounded = _clamp(v, 0, SCORE)
    return ((bounded + CONSENSUS_STEP // 2) // CONSENSUS_STEP) * CONSENSUS_STEP
...
class CrucibleCourt(gl.Contract):
    ...
    def evaluate_round(self, study_id: u32) -> None:
        ...
        def judge_fn():
            ...
            for cid in known:
                norm[cid]["credence"] = _quantize(norm[cid]["credence"])
            return {"verdicts": [{"claim_id": k, "tier": v["tier"], "credence": v["credence"]} for k, v in norm.items()],
                    "attack_strength": _quantize(_as_int(out.get("attack_strength"))),
        ...
        def judge_validator(res: gl.vm.Result) -> bool:
            ...
            mine_raw = judge_fn()
            mine = _norm_verdicts(mine_raw, known)
            # Every field that can change canonization, retraction, or a payout
            # is quantized by the producer and compared exactly by validators.
            for cid in known:
                if leader[cid]["tier"] != mine[cid]["tier"]:
                    return False
                if int(leader[cid]["credence"]) != int(mine[cid]["credence"]):
                    return False
            try:
                leader_attack = int(d.get("attack_strength"))
                mine_attack = int(mine_raw.get("attack_strength"))
            except Exception:
                return False
            return leader_attack == mine_attack
        ...
        if pending:
            was_canonized = int(s.ever_canonized) == 1 and int(s.pre_credence) >= CANON_CREDENCE
            flipped = was_canonized and (int(s.credence) < CANON_CREDENCE or not all_replicated)
        ...
        if int(s.rounds) >= ROUNDS_TO_CANONIZE and int(s.credence) >= CANON_CREDENCE and int(s.survived_attack) == 1 and all_replicated:
            s.status = ST_CANONIZED
            refund = 0
            if int(s.ever_canonized) == 0:
                bonded = int(s.bond)
                reserve = (bonded * CANON_RESERVE_BPS) // 10000
                refund = bonded - reserve
                s.bond = u256(reserve)
                s.ever_canonized = u8(1)
